...
export function shouldShow(verdict, settings) {
  ...
  // Estimate-confidence verdicts (no probability distribution: heuristics,
  // chat-estimate) must clear the threshold by 0.25 to earn a badge — except
  // heuristic production fallbacks, which would otherwise never show the very
  // fabrication signals this taxonomy added. Documented tradeoff: heuristic
  // badges are noisier, marked estimated in the tooltip.
  const bar = (verdict.estimated && verdict.source !== 'heuristic'
