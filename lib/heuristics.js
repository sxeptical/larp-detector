...
export function analyzeHeuristically({ post_text = '', author_headline = '' }) {
  ...
  const imageCrafted = clamp01(
    (announceOrBrag || wisdomList || mundane || bait || cliffhanger) * 0.5 +
  ...
  // headline? Reuse the receipts signals; an empty/vague post cannot support.
  const headlineSupported = clamp01(
    0.6 * workShown + 0.2 * (lineCount >= 6 ? 1 : 0) + 0.2 * (announce ? 0.5 : 0)
  );
  ...
  const storyVividness = clamp01(
    0.4 * mundane + 0.3 * (lineCount >= 8 ? 1 : 0) + 0.3 * hit(t, /\bstory\b|when i was|years ago/i)
  );
    ...
    answers: {
      ...
      larp_intensity: {
        ...
        score: Number(intensityFor({
          ...
          verifiableStory, borrowedContent, virtuePerformance,
        }).toFixed(2)),
        legend: {
...
function intensityFor({
  ...
  // checked; applause imported with nothing of the author added.
  intensity += 1.2 * (1 - verifiableStory) * storyVividBoost(arguments[0]);
...
function storyVividBoost({ cliffhanger, mundane }) {
  return cliffhanger || mundane ? 1 : 0.5;
}
