...
export function analyzeHeuristically({ post_text = '', author_headline = '' }) {
  ...
  const wisdom = clamp01(mundane && lesson ? 1 : lesson && wisdomList > 0.5 ? 0.7 : 0);
  ...
  const persona = clamp01(
    0.25 * wisdom + 0.25 * grind + 0.2 * metrics + 0.2 * (grandiose * (1 - specifics)) +
  ...
  const intensityConfidence = Math.max(...Object.values(levelProbs));
  ...
  return {
    answers: {
      ...
      larp_intensity: {
        ...
        confidence: Number(intensityConfidence.toFixed(3)),
      },
    },
