 ...
 *
 * Providers:
 *   typesafe    — direct API (default). POST /v1/systemone, Jev's native
 *                 state/questions schema. https://api.typesafe.ai
 *   openrouter  — LLM emulation. Jev itself is NOT served by OpenRouter yet
 *                 (placeholder listing only; verified against /api/v1/models,
 *                 Sep 19 2026). Instead, a cheap chat model answers the exact
 *                 same question set in structured JSON, mapped back into Jev's
 *                 answer shape so the rest of the pipeline is unchanged.
...
function resolveModel(provider, model) {
  ...
  if (provider === 'openrouter' && !m.includes('/')) return fallback;
  if (provider === 'typesafe' && m.includes('/')) return fallback;
  return m;
}
...
export async function callJev(state, settings) {
  ...
  if (provider === 'openrouter') {
    return callOpenRouter(state, settings, apiKey);
  }
...
async function callOpenRouter(state, settings, apiKey) {
  const model = resolveModel('openrouter', settings.model);
